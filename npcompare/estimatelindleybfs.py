    ...
    **kwargs:
    ...
    if self.__isinitialized:
      ...
      self.bfs2 = EstimateBFS(obs2, nmaxcomp, hpp, hpgamma,
                              transformation, self._ismixture,
                              **kwargs)
      self.bfsconcat = EstimateBFS(obsconcat, nmaxcomp, hpp, hpgamma,
                                   transformation, self._ismixture,
                                   **kwargs)
    ...
    if self._smodel is None:
      if self._ismixture:
        if EstimateLindleyBFS._smodel_mixture is None:
          EstimateLindleyBFS._smodel_mixture = pystan.StanModel(model_code = EstimateLindleyBFS._smodelcode_mixture)
        self._smodel = EstimateLindleyBFS._smodel_mixture
      else:
        if EstimateLindleyBFS._smodel_single is None:
          EstimateLindleyBFS._smodel_single = pystan.StanModel(model_code=EstimateLindleyBFS._smodelcode_single)
  ...
  def _processfit(self):
    if not self.sfit:
      raise Exception("This function cannot be called before "
                      "obj.sampleposterior()")
    self.bfs1.sfit = self.bfs2.sfit = self.bfsconcat.sfit = self.sfit
